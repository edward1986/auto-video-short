...
start_index = next(
    (i for i, line in enumerate(readme_content) if start_marker in line), None
)
end_index = next(
    (i for i, line in enumerate(readme_content) if end_marker in line), None
)
...
for post in latest_posts:
    ...
    if post["video_id"]:
        new_content.append(
            f"[🎥 Watch Video](https://www.youtube.com/watch?v={post['video_id']})\n"
        )
    new_content.append("\n---\n")
...
if start_index is not None and end_index is not None:
    updated_content = (
        readme_content[: start_index + 1] + new_content + readme_content[end_index:]
    )
else:
