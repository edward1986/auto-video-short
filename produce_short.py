...
def produce_short(
    ...
    background = resize(
        (
            ...
            .cutout(0, max(1, round(background_duration) - 65))  # ✅ Ensure valid range
    ...
    music_start_time = max(1, min(available_music_time, randint(1, int(available_music_time))))

    music = volumex(
        editor.CompositeAudioClip([
            editor.AudioFileClip(music)
            .cutout(0, music_start_time)
            .set_end(full_question_duration)
        ]),
        0.6
    )
    ...
    question_text = editor.TextClip(
        ...
        size=(1080, None),
        font=font
    ).set_position(("center", 0.03), relative=True).set_start(0).set_duration(clip_durations["question"])
    ...
    answer_texts = [
        editor.TextClip(
            ...
            size=(1080, None),
            font=font
        ).set_position(("center", 0.35 + (i / 7)), relative=True).set_start(0).set_duration(clip_durations["question"])
    ...
    countdown_texts = [
        editor.TextClip(
            ...
            size=(1080, None),
            font=font
        ).set_start(i).set_duration(1).set_position(("center", 0.87), relative=True)
        for i in range(clip_durations["question"])
    ]
    clips += countdown_texts

    # ✅ Highlight the correct answer
    correct_answer_text = editor.TextClip(
        ...
        size=(1080, None),
        font=font
    ).set_start(clip_durations["question"]).set_duration(clip_durations["answer"]).set_position("center")
